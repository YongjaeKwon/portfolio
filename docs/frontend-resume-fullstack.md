...
화면 구현을 중심으로 일해왔지만, 실제 업무에서는 API, Service, MyBatis SQL, 인증, 메일·파일·알림 연동까지 함께 확인하며 기능을 개발해온 웹 개발자입니다.

B2B·B2G 운영 시스템에서 Vue, WebSquare, JSP, Nexacro 화면과 Java/Spring API, MyBatis SQL을 함께 다뤘습니다. 사용자가 보는 화면과 서버에서 조회·저장되는 데이터가 같은 기준으로 동작하는지 확인하며, 권한, 조직, 기간, 상태값에 따른 오류를 재현하고 수정하는 데 강점이 있습니다.

개인 프로젝트에서는 FastAPI, PostgreSQL, Redis, WebSocket, Docker Compose를 사용해 백엔드 구조와 실시간 데이터 흐름을 학습하고 있습니다.

## Core Skills

- Frontend: HTML5, CSS3, JavaScript, TypeScript, Vue.js, React, JSP, WebSquare, Nexacro, jQuery
...
- Tools: Git, SVN, Maven, Gradle, Docker, Docker Compose, Nginx, Vite, TailwindCSS
- Strength: 화면/API/SQL 동시 점검, 권한·조회 조건, 파일·메일·인증 연동, 운영 이슈 대응
...
협력사, CE, 교육, 계약, 공지, 설문 데이터를 관리하는 Spring Boot 3 기반 운영 포탈입니다. Vue 화면부터 Spring Boot Controller/Service, MyBatis SQL, 메일·엑셀·OTP 연동까지 함께 다룬 대표 프로젝트입니다.
...
AS 접수, 상담, 이관, 배정, 배송, 개인정보 동의, QR 확인, 태블릿 전자서명 업무를 처리하는 WebSquare/JSP 기반 운영 시스템입니다. 모바일·태블릿 화면과 Java/Spring MVC API, 알림톡·파일·서명 흐름을 함께 확인했습니다.
...
FastAPI 백엔드와 React/TypeScript 대시보드를 6개 컨테이너로 분리해 시장 데이터 수집, 전략 백테스트, 실거래 실행 구조를 학습하는 프로젝트입니다.
