...
1. PPS
   - Vue, Spring Boot, 관리자 화면, 인증, 파일/메일/조회 흐름이 있어 실무 FE 근거가 가장 강함
2. TSMS
   - 웹/모바일/태블릿 업무 화면 경험이 있어 사용자 접점이 있는 업무 흐름 설명에 적합함
3. 교육청 IT 자산관리
   - 권한별 조회 조건, SQL, 외부 API 연동 이해를 보여주는 실무 보조 카드
4. SR30
   - Nexacro 중심이라 최신 FE 직접성은 낮지만 운영 조건, 통계, 리포트, 이력 조회 경험을 보조로 설명하기 좋음
5. quant-core
   - React/TypeScript/Zustand/WebSocket/Docker 보완용. 단, 학습 프로젝트이므로 실무 뒤에 배치
6. SSAFAST
...
- SR30
  - 응답률 개선 수치는 실제 집계 근거가 확인되기 전까지 사용하지 않음
  - "설문 등록·응답·통계 흐름 개선", "운영자가 응답 현황을 확인할 수 있도록 보완" 정도로 완화
