...
요구사항을 기능 목록으로만 보지 않고, 운영자가 어떤 업무를 끝내야 하는지와 어떤 데이터를 다시 확인해야 하는지를 함께 봅니다. 화면 수정, API 연동, 데이터 조회, 엑셀/메일/알림 연동, 운영 이슈 대응을 해온 경험을 바탕으로 새로운 도메인의 운영 정책도 빠르게 학습해 적용할 수 있습니다.

## Core Skills

- 운영 기능 개발: 관리자 화면, 공지, 설문, 교육, 발송, 엑셀 업로드/다운로드, 이력 조회
- 데이터 확인: 권한, 조직, 기간, 상태값에 따른 MyBatis SQL과 화면 조회 조건 점검
- 외부 연동: 메일, 알림톡/BizTalk, 파일, QR, OTP, 전자서명 결과를 화면 상태와 연결
- Frontend: JavaScript, TypeScript, Vue.js, React, JSP, WebSquare, Nexacro, jQuery
- Backend/Data: Java, Spring Boot, Spring MVC, MyBatis, MariaDB, Oracle, FastAPI, Python
...
- 운영자가 확인해야 하는 발송 결과, 업로드 결과, 이력 데이터를 화면에 반영
...
협력사, CE, 교육, 계약, 공지, 설문 데이터를 관리하는 Spring Boot 3 기반 운영 포탈입니다. 대상자 관리, 공지, 설문, 발송, 엑셀, 제출 현황이 이어지는 반복 운영 업무 흐름을 다뤘습니다.

- 교육 등록, 대상자 엑셀 업로드, 메일 발송, 제출 현황 조회 흐름 구현
- 게시판·제안하기·설문 댓글/대댓글 UI와 처리 흐름 공통화
...
AS 접수, 상담, 이관, 배정, 배송, 개인정보 동의, QR 확인, 태블릿 전자서명 업무를 처리하는 운영 시스템입니다. 현장 사용자와 내부 담당자가 같은 업무 상태를 기준으로 움직일 수 있도록 화면 흐름을 구현했습니다.
...
사용 기술: React, TypeScript, Zustand, FastAPI, Python, PostgreSQL, Redis, Docker, WebSocket
