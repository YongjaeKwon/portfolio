...
- 외부 연동: 메일, 외부 메시지 발송, 파일, 엑셀, QR, 인증, 전자서명 결과를 화면 상태와 연결
- Production Frontend: Vue.js, JavaScript, WebSquare, JSP, Nexacro, jQuery
...
협력사, 고객사 담당자, 교육, 계약, 공지, 설문 데이터를 관리하는 Spring Boot 3 기반 운영 포탈입니다. Vue 화면부터 Spring Boot API, MyBatis SQL, 메일·엑셀·인증 연동까지 같은 교육·대상자 기준으로 맞춰야 하는 대표적인 API & Data 경험입니다.
