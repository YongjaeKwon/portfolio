...
Vue 기반 관리자 화면과 운영 업무 UI를 주력으로 개발해왔고, API 응답·권한 조건·MyBatis 조회 기준까지 함께 확인해 화면 결과의 정합성을 맞춰온 프론트엔드 중심 웹 개발자입니다.

Vue, WebSquare, JSP, Nexacro 기반 실무 화면을 개발했고, Spring Boot/Spring MVC API와 SQL 조회 조건까지 함께 확인하며 화면과 데이터가 같은 기준으로 동작하도록 구현했습니다. 운영자가 반복하는 등록, 업로드, 발송, 조회, 이력 확인 흐름을 안정적으로 만드는 데 강점이 있습니다.
