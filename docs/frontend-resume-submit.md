...
Vue 기반 관리자 화면을 주력으로 개발해온 프론트엔드 개발자입니다. 화면 구현에 그치지 않고 API 응답·권한 조건·MyBatis 조회 기준까지 직접 대조하며, 화면에 보이는 값과 실제 데이터가 어긋나지 않도록 맞춰 왔습니다.

## Core Skills

- Production Frontend: Vue.js, JavaScript, WebSquare, JSP, Nexacro, jQuery, HTML5, CSS3
- Project Frontend: React, TypeScript, Next.js, Zustand, Pinia, Redux, React Query, React Hook Form, Canvas
- Backend: Java, Spring Boot, Spring MVC, Spring Security, MyBatis, REST API, JWT, WebSocket, Python, FastAPI (개인 프로젝트(quant-core))
...
- Strength: 관리자·운영 화면, 업무 단계형 화면, 권한/조회 조건, 파일 업로드, 메일/알림 발송, 엑셀 다운로드, 운영 이슈 대응
...
- 조회 조건, 권한 반영, 입력 검증, 상태값 불일치 문제를 재현하고 API 응답·SQL 결과를 직접 대조
...
협력사, 고객사 담당자, 교육, 계약, 공지, 설문 데이터를 관리하는 Spring Boot 3 기반 운영 포탈입니다. Vue 화면과 Tabulator 그리드, Spring Boot API, MyBatis SQL을 연결해 운영자가 반복해서 사용하는 교육관리와 협력사 관리 흐름을 개발했습니다.
...
AS 접수, 상담, 이관, 배정, 배송, 개인정보 동의, QR 확인, 태블릿 전자서명 업무를 처리하는 WebSquare/JSP 기반 운영 시스템입니다. 데스크톱, 모바일, 태블릿 화면이 업무 번호와 접수 상태를 기준으로 이어지도록 구현했습니다.
...
- 권한 또는 센터 조건 변경 후 목록과 상세 화면의 조회 범위를 직접 확인
...
- 화면 필터, SQL 조건, 엑셀 다운로드 결과가 맞아떨어지는지 직접 대조
...
사용 기술: React, TypeScript, Redux, Vite, TailwindCSS, Canvas, TensorFlow

### 퀀트 트레이딩 시스템 (quant-core)

2026.03 ~ 현재 · 개인 학습 프로젝트

시뮬레이션 기반 백테스트와 실시간 대시보드 구조를 학습하기 위해 진행 중인 개인 프로젝트입니다. FastAPI 백엔드와 React/TypeScript 대시보드를 6개 컨테이너로 분리해 서비스 구조를 익히고 있습니다.

- JWT 인증과 Redis 블랙리스트 기반 로그아웃, IP별 로그인 실패 제한 구현
- WebSocket으로 포지션·체결 결과를 프론트에 실시간 브로드캐스트
- Walk-Forward 검증과 백테스트 실행 화면 구현, 수수료·펀딩비를 포함한 시뮬레이션 기준 net PnL 기록
