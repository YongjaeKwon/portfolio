...
B2B·B2G 운영 시스템에서 Spring MVC API와 MyBatis 데이터 접근 계층을 직접 구현해온 웹 개발자입니다. Controller·Service·Mapper로 요청부터 응답까지 처리하고, 권한·상태값·기간 조건에 따른 동적 SQL과 집계 쿼리가 어긋나지 않는지 직접 대조합니다. 메일·파일·인증·알림처럼 실패가 잦은 외부 연동도 결과 응답까지 서버에서 책임집니다.

## Core Skills

- API/Server: Java, Spring Boot, Spring MVC, Spring Security, REST API, Controller·Service·Mapper, JWT, WebSocket
- Data Access: MyBatis, 동적 SQL, 권한·조직·기간·상태값 조건 처리, 집계 쿼리 검증
- Database: MariaDB, Oracle, PostgreSQL, Redis, SQLite, PL/SQL
- External Integration: 메일, 외부 메시지 발송, 파일, 엑셀, QR, 인증, 전자서명 결과 처리
- Frontend Context: Vue.js, JavaScript, WebSquare, JSP, Nexacro, jQuery, React, TypeScript, Next.js
...
- B2B·B2G 운영 시스템의 API, SQL, 화면, 외부 연동 기능 개발
- Spring Boot 3, Spring MVC 환경에서 Controller·Service·Mapper 흐름과 MyBatis SQL 조건 구현
- 권한·조직·기간·상태값 조건에 따른 조회 범위와 집계 결과를 직접 대조
- 발송 결과, 업로드 결과, 엑셀 다운로드, 이력 데이터가 요구사항과 어긋나지 않도록 응답 처리 개선
- Vue, WebSquare, JSP, Nexacro 기반 관리자·모바일 화면과 서버 응답 연결 및 운영 오류 재현
...
협력사, 고객사 담당자, 교육, 계약, 공지, 설문 데이터를 관리하는 Spring Boot 3 기반 운영 포탈입니다. Vue 화면부터 Spring Boot API, MyBatis SQL, 메일·엑셀·인증 연동까지 교육·대상자 키를 한 기준으로 묶어 구현한 대표 프로젝트입니다.
...
- 대시보드 클릭 이후 목록 조회 결과가 집계 수치와 맞아떨어지는지 직접 확인
...
AS 접수, 상담, 이관, 배정, 배송, 개인정보 동의, QR 확인, 태블릿 전자서명 업무를 처리하는 운영 시스템입니다. 모바일·태블릿 화면 요청을 Spring MVC API와 MyBatis SQL, 알림톡·파일·서명 처리 흐름으로 연결했습니다.
...
- 화면 필터, SQL 조건, 엑셀 다운로드 결과를 직접 비교해 조회 조건 누락을 추적
